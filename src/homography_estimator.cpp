...
bool HomographyEstimator::initialize() {
    ...
    if(config().get<bool>("outline_points_click", true)) {
        ...
        auto outlineX = config().getArray<int>("outline_points_x");
        auto outlineY = config().getArray<int>("outline_points_y");
    ...
    blobDetector = std::make_shared<cv::SimpleBlobDetector>(blobParams);
...
bool HomographyEstimator::cycle() {
    ...
    cv::imshow("hompgraphy_estimator_input", imgColor);
    ...
    if(!estimate.empty() && topView2cam.empty()){
        ...
        // Detect pattern
        detectPattern(warpedColor, warpedColor, detectedPoints);
...
void HomographyEstimator::userClickCallback(int e, int x, int y, int d)
{
    ...
    if(estimatePoints.size() == 4) {
        // Compute hompgraphy estimate
        computeEstimate();
    }
}

void HomographyEstimator::detectPattern(cv::Mat& img, cv::Mat& visualization, std::vector<cv::Point2f>& points)
{
    auto found = findPoints(img, points);
...
bool HomographyEstimator::findPoints(const cv::Mat& img, std::vector<cv::Point2f>& points)
{
    ...
    switch (pattern) {
        ...
        case Pattern::CIRCLES_ASYMMETRIC:
        {
            cv::Ptr<cv::FeatureDetector> blobDetector = new cv::SimpleBlobDetector(getBlobDetectorParams());
...
void HomographyEstimator::computeRefinement()
{
